"""
Script Name: build_SWmainInventory.py
Author: Nicholas Chang
Created: 2026-06-03
Last Updated: 2026-06-03
Purpose:
    Build batched USGS NWIS inventory request URLs for surface-water stations
    and download the combined surface-water inventory table.

Project:
    ArcNLET Charlotte Calibration Data - USGS Surface-Water Inventory

Notes:
    - Reads cleaned surface-water station metadata.
    - Extracts unique USGS site numbers.
    - Builds NWIS Site Service inventory URLs in batches of 30 sites.
    - Uses seriesCatalogOutput=true to request the available data-series inventory
      for each surface-water station.
...
INPUT_FILE = Path(
    r"C:\ArcNLET-CharlotteCalibrationData\USGS_Data\USGS_GWandSW_Locations\Data\SecondTry\01_metadata\SW\SW_StationMetadata_20260603_154346.csv"
)
...
OUTPUT_FOLDER = BASE_INVENTORY_FOLDER / "SW"
...
def main() -> None:
    """Run the surface-water inventory URL-building and download workflow."""

    run_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    OUTPUT_FOLDER.mkdir(parents=True, exist_ok=True)

    url_csv = OUTPUT_FOLDER / f"SW_MainInventory_URLs_{run_timestamp}.csv"

    inventory_csv = OUTPUT_FOLDER / f"SW_MainInventory_{run_timestamp}.csv"

    print("Starting surface-water main inventory build.")
    ...
    print("Surface-water main inventory build complete.")
    print(f"Site-number column used: {site_number_column}")
    print(f"Unique surface-water stations: {len(site_numbers)}")
