"""
Script Name: get_GWvalues.py
Author: Nicholas Chang
Created: 2026-06-05
Last Updated: 2026-06-05
Purpose:
    Download USGS groundwater values for filtered groundwater inventory CSVs.

    This script reads the filtered groundwater inventory CSVs created by
    filter_GWinventory.py, then downloads values from the appropriate USGS
    endpoint based on data_type_cd:

        gw = USGS modern Field Measurements API
    ...
    The script writes:
        ...
        2. One combined values CSV containing all downloaded groundwater values.
        3. One request log CSV documenting every request.
        4. Optional raw-response files when SAVE_RAW_RESPONSES = True.

Project:
    ArcNLET Charlotte Calibration Data - USGS Groundwater Values

Notes:
    - gw records are irregular field measurements, not daily statistics.
...
INPUT_FOLDER = Path(
    r"C:\ArcNLET-CharlotteCalibrationData\USGS_Data\USGS_GWandSW_Locations\Data\SecondTry\03_filtered\GW"
)
...
OUTPUT_FOLDER = OUTPUT_PARENT_FOLDER / "GW"

# Modern USGS OGC API endpoint used for data_type_cd = gw.
FIELD_MEASUREMENTS_SERVICE_URL = (
...
FIELD_MEASUREMENTS_PAGE_LIMIT = 50000
...
REQUEST_LOG_CSV = OUTPUT_FOLDER / f"GW_VALUES_request_log_{RUN_TIMESTAMP}.csv"

COMBINED_VALUES_CSV = OUTPUT_FOLDER / f"GW_VALUES_COMBINED_{RUN_TIMESTAMP}.csv"
...
def normalize_monitoring_location_id(value: object) -> str:
    """
    Convert a USGS site number into a modern monitoring_location_id.
    """
    ...
    if site_no.upper().startswith("USGS-"):
        return site_no

    if site_no.upper().startswith("USGS:"):
        return "USGS-" + site_no.split(":", 1)[1]

    return f"USGS-{site_no}"


def site_no_from_monitoring_location_id(value: object) -> str:
    """
    Convert a monitoring_location_id such as USGS-02238500 into site_no.
    """
    text = str(value).strip()

    if "-" in text:
        return normalize_site_number(text.split("-", 1)[1])

    if ":" in text:
        return normalize_site_number(text.split(":", 1)[1])

    return normalize_site_number(text)


def agency_from_monitoring_location_id(value: object) -> str:
    """
    Extract the agency prefix from a monitoring_location_id.
    """
    text = str(value).strip()
...
def flatten_json_value(value: object) -> str:
    """
    Convert nested JSON values into safe CSV-friendly strings.
    """
    if value is None:
        return ""
...
def get_filtered_csv_files() -> list[Path]:
    """
    Return the filtered groundwater CSVs that should be processed.
...
def add_normalized_filter_fields(filtered_df: pd.DataFrame) -> pd.DataFrame:
    """
    Add normalized internal fields for data_type_cd, parm_cd, and stat_cd.
    """
    ...
    for attempt_number in range(1, MAX_RETRIES + 1):
        try:
            response = requests.get(
                ...
                headers={
                    "User-Agent": "FREAC-USGS-GW-Values-Downloader/1.0",
...
def build_field_measurements_url(
    ...
    """
    Build one modern USGS Field Measurements API URL for gw records.
    """
    monitoring_location_ids = [
        normalize_monitoring_location_id(site_no)
        for site_no in batch_sites
    ]

    params = {
        "f": "json",
        "lang": "en-US",
        "monitoring_location_id": ",".join(monitoring_location_ids),
        "parameter_code": parm_cd,
        "limit": FIELD_MEASUREMENTS_PAGE_LIMIT,
        "offset": offset,
        "skipGeometry": "true",
    }

    if start_dt and end_dt:
        params["time"] = f"{start_dt}T00:00:00Z/{end_dt}T23:59:59Z"

    query_string = urlencode(params, safe=",-:/")

    return f"{FIELD_MEASUREMENTS_SERVICE_URL}?{query_string}"
...
# ---------------------------------------------------------------------------
# MODERN FIELD MEASUREMENTS JSON PARSING FUNCTIONS
# ---------------------------------------------------------------------------

def parse_field_measurements_json(
    json_text: str,
    requested_parm_cd: str,
) -> pd.DataFrame:
    """
    Parse a modern USGS Field Measurements API JSON response into a long table.
    """
    ...
    for feature in features:
        ...
        site_no = site_no_from_monitoring_location_id(monitoring_location_id)

        measured_value = str(properties.get("value", "")).strip()

        if DROP_EMPTY_VALUE_ROWS and not measured_value:
            continue

        parm_cd = clean_code(
            properties.get("parameter_code", requested_parm_cd),
            width=5,
        )

        record = {
            "agency_cd": agency_from_monitoring_location_id(monitoring_location_id),
            "site_no": site_no,
            "station_nm": "",
            "datetime": str(properties.get("time", "")).strip(),
            "time_zone": "",
            "value": measured_value,
            "qualifier": flatten_json_value(properties.get("qualifier", "")),
            "approval_status": flatten_json_value(
                properties.get("approval_status", "")
            ),
            "parm_cd": parm_cd if parm_cd else requested_parm_cd,
            "stat_cd": "",
            "time_series_id": "",
            "parameter_name": "",
            "unit_code": str(properties.get("unit_of_measure", "")).strip(),
            "source_value_column": "features.properties.value",
        }
        ...
        for key, value in properties.items():
            ...
            record[f"field_{key}"] = flatten_json_value(value)
...
def download_field_measurements_for_batch(
    input_csv: Path,
    batch_sites: list[str],
    site_metadata_df: pd.DataFrame,
    data_type_cd: str,
    ...
    """
    Download all paginated field-measurement records for one site batch.
    """
    value_frames = []
    request_records = []
    ...
    while True:
        page_number = f"{page_index:03d}"

        request_url = build_field_measurements_url(
        ...
        try:
            ...
            values_df = parse_field_measurements_json(
            ...
            if len(values_df) < FIELD_MEASUREMENTS_PAGE_LIMIT:
                break

            offset += FIELD_MEASUREMENTS_PAGE_LIMIT
            page_index += 1
    ...
    return value_frames, request_records
...
def download_values_for_group(
    ...
    for batch_index, batch_sites in enumerate(batches, start=1):
        ...
        if data_type_cd == "gw":
            value_frames, request_records = download_field_measurements_for_batch(
                input_csv=input_csv,
                batch_sites=batch_sites,
                site_metadata_df=site_metadata_df,
                data_type_cd=data_type_cd,
            ...
            request_record["error_message"] = (
                "Only data_type_cd values of gw, dv, uv, and iv are supported."
            )
...
def process_filtered_csv(input_csv: Path) -> tuple[pd.DataFrame, list[dict[str, object]]]:
    """
    Process one filtered groundwater inventory CSV.
    """
...
def main() -> None:
    """
    Run the full groundwater values download workflow.
    """
    print("Starting USGS groundwater values download workflow.")
    ...
    if not filtered_csv_files:
        raise FileNotFoundError(
            f"No filtered groundwater CSV files were found in: {INPUT_FOLDER}"
        )
    ...
    if combined_value_frames:
        ...
        print("Combined groundwater values CSV written.")
        print(f"Combined rows: {len(combined_values_df):,}")
        print(f"Combined CSV: {COMBINED_VALUES_CSV}")

    else:
        print("No groundwater values were downloaded. Combined CSV was not written.")

    print("Groundwater values download workflow complete.")
